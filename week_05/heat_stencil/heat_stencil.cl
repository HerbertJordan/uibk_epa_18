...
__kernel void stencil(
    __global const value_t* A,      // the source buffer
    __global value_t* B,            // the target buffer
    int source_x,                   // the location of the heat source (x-coordinate)
    int source_y,                   // the location of the heat source (y-coordinate)
    int N,                          // the width of the room
    __local value_t* L              // local memory to speed up computatoin
) {
    ...
    if( i < N && j  < N ) {
        ...
        if (li == mi-1) L(li+1,lj) = G(i+1,j);
    
        if (lj ==   0 ) L(li,lj-1) = G(i,j-1);
        if (lj == mj-1) L(li,lj+1) = G(i,j+1);
    }
